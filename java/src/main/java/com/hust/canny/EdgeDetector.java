 ...
 * ①对图像进行高斯滤波平滑
 * ②计算滤波后图像梯度的幅值和方向
 * ③对梯度幅值应用非极大值抑制。其过程是找出图像梯度中的局部极大值点， 把其他非局部极大值点置零以得到细化的边缘。
 * ④再用双域值算法检测和连接边缘，删除非显著的边缘。
 * 
 * @author 刘翔宇
 * 
 */
...
public class EdgeDetector extends Component {
	...
	public void process() throws EdgeDetectorException {
		...
		if (threshold < 0 || threshold > 255)
			throw new EdgeDetectorException(
					"The value of the threshold is out of its valid range.");
		if (widGaussianKernel < 3 || widGaussianKernel > 40)
			throw new EdgeDetectorException(
					"The value of the widGaussianKernel is out of its valid range.");
		width = sourceImage.getWidth(this); // 获取源图片的宽
		height = sourceImage.getHeight(this); // 获取源图片的高
		picsize = width * height;
		data = new int[picsize];
		magnitude = new int[picsize];
		orientation = new int[picsize];
		/*
		 * σ为高斯滤波器参数，它控制了高斯滤波器的平滑程度。σ较小的滤波器， 边缘的定位精度比较高，但是图像的信噪比较低； σ 较大的滤波器，
		 * 则能较好的平滑噪声，但是降低了边缘定位精度。
		 */
		
		float f = Constant.SIAMA;// f为高斯滤波器参数
		...
		for (int i = 0; i < picsize; i++) {
			if (data[i] < threshold)
				data[i] = 0xff000000;// -16777216=0xff000000,黑色,非边缘点
			else
				data[i] = 0xffffffff;// -1=0xffffffff,白色,边缘点
		}
		edgeImage = pixels2image(data);// 得到边缘图片
		magnitude = null;
		orientation = null;
	}
	...
	private void canny_core(float f, int i) {
		...
		for (int k5 = j; k5 < j1; k5++) {
			for (int k6 = l; k6 < i1; k6 += width) {
				...
				float f5;
				if (f6 * f7 <= (float) 0 ? Math.abs(f6) >= Math.abs(f7) ? (f5 = Math
						.abs(f6 * f12)) >= Math.abs(f7 * f18 - (f6 + f7) * f16)
						&& f5 > Math.abs(f7 * f19 - (f6 + f7) * f15)
						: (f5 = Math.abs(f7 * f12)) >= Math.abs(f6 * f18
								- (f7 + f6) * f13)
								&& f5 > Math.abs(f6 * f19 - (f7 + f6) * f14)
						: Math.abs(f6) >= Math.abs(f7) ? (f5 = Math.abs(f6
								* f12)) >= Math.abs(f7 * f20 + (f6 - f7) * f16)
								&& f5 > Math.abs(f7 * f17 + (f6 - f7) * f15)
								: (f5 = Math.abs(f7 * f12)) >= Math.abs(f6
										* f20 + (f7 - f6) * f14)
										&& f5 > Math.abs(f6 * f17 + (f7 - f6)
												* f13)) {
